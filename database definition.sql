...
@sid Int
AS
SELECT Course.name FROM Course LEFT OUTER JOIN StudentTakeCourse ON Course.id = StudentTakeCourse.cid and StudentTakeCourse.sid=@sid
...
go
CREATE PROC  PromosIdontHave  @id int, @sid int
AS
SELECT code, isuueDate, expiryDate, discount, adminId FROM Promocode AS P WHERE (adminId = @id) AND (NOT EXISTS (SELECT sid, code FROM StudentHasPromocode AS S WHERE (P.code = code) AND (sid = @sid)));
